...
import type { TransformImport } from '@rsbuild/webpack/src/types';
...
export const applyWebpackBabelConfig = (
  ...
  api.modifyBundlerChain(
    async (
      chain,
      { CHAIN_ID, target, isProd, isServer, isServiceWorker, getCompiledPath },
    ) => {
      ...
      const getBabelOptions = (config: SharedNormalizedConfig) => {
        ...
        const decoratorConfig = {
          version: config.output.enableLatestDecorators ? '2018-09' : 'legacy',
        } as const;
        ...
        applyPluginLodash(baseBabelConfig, config.performance.transformLodash);

        const babelConfig = mergeChainedOptions(baseBabelConfig, userOptions, {
